...
class IBDScreeners:
    ...
    def calculate_rs_rating(self, prices_df, benchmark_prices_df=None):
        """
        IBD スタイルの RS Rating を計算

        Formula: 0.4 * ROC(63d) + 0.2 * ROC(126d) + 0.2 * ROC(189d) + 0.2 * ROC(252d)

        Args:
            prices_df: 株価データ
            benchmark_prices_df: ベンチマーク（S&P 500など）の株価データ

        Returns:
            float: RS Rating (0-100)
        """
    ...
    def calculate_eps_growth(self, income_statements):
        """
        EPSの成長率を計算（前年同期比）

        Args:
            income_statements: 損益計算書データ（四半期）

        Returns:
            dict: 最新四半期と次四半期のEPS成長率
        """
        ...
        try:
            # 最新の四半期EPS
            latest_eps = income_statements[0].get('eps', 0)
            # 1年前の同四半期EPS（4四半期前）
            yoy_eps = income_statements[4].get('eps', 0)

            if yoy_eps != 0:
                eps_growth_last_qtr = ((latest_eps - yoy_eps) / abs(yoy_eps)) * 100
            else:
    ...
    def estimate_ad_rating(self, prices_df):
        """
        A/D Rating（Accumulation/Distribution）の簡易推定

        IBDの正確な計算は不可能だが、価格とボリュームの関係から推定
        13週間のデータで、価格上昇時の高ボリュームを評価
        ...
        try:
            ...
            volume_normalized = (volume[1:] - np.mean(volume)) / np.std(volume)

            # 価格上昇日の平均ボリューム vs 価格下落日の平均ボリューム
            up_days = price_changes > 0
            down_days = price_changes < 0

            if np.sum(up_days) > 0 and np.sum(down_days) > 0:
                avg_vol_up = np.mean(volume[1:][up_days])
                avg_vol_down = np.mean(volume[1:][down_days])

                ratio = avg_vol_up / avg_vol_down if avg_vol_down > 0 else 1
    ...
    def estimate_comp_rating(self, rs_percentile, eps_growth, ad_rating, price_vs_52w_high):
        """
        Composite Rating の簡易推定

        IBDの正確な計算は不可能だが、主要要素（RS, EPS, A/D）から推定
        ...
        try:
            ...
            if rs_percentile is not None:
                score += rs_percentile * 0.4

            # EPS Growth (30% weight)
            if eps_growth is not None and eps_growth > 0:
                eps_score = min(eps_growth, 100)  # Cap at 100
                score += eps_score * 0.3

            # A/D Rating (20% weight)
            ad_score_map = {'A': 100, 'B': 75, 'C': 50, 'D': 25, 'E': 0}
            if ad_rating and ad_rating in ad_score_map:
                score += ad_score_map[ad_rating] * 0.2

            # Distance from 52-week high (10% weight)
            if price_vs_52w_high is not None:
                # -10% or better = 100, -50% or worse = 0
                high_score = max(0, min(100, (price_vs_52w_high + 50) * 2.5))
    ...
    def screener_healthy_chart_watchlist(self, tickers_list, rs_percentile_dict, benchmark_prices_df):
        ...
        for ticker in tickers_list:
            try:
                ...
                eps_metrics = self.calculate_eps_growth(income_stmt)
                if eps_metrics:
                    eps_growth = eps_metrics['eps_growth_last_qtr']

                price_vs_52w = self.calculate_52w_high_distance(prices_df)
                comp_rating = self.estimate_comp_rating(rs_rating, eps_growth, ad_rating, price_vs_52w)
